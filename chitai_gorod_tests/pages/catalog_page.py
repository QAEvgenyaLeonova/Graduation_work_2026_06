...
class CatalogPage:
    ...
    def __init__(self, driver: WebDriver):
        ...
        try:
            ...
            self.test_data = {
                "search_phrases": ["Пушкин"],
                "no_result_phrase": "xyz",
            }
    ...
    def _wait_for_results(self) -> None:
        try:
            self.wait.until(
                lambda driver: (
                    len(driver.find_elements(By.XPATH, self.PRODUCT_CARD)) > 0
                    or len(driver.find_elements(By.XPATH, self.NO_RESULTS)) > 0
                )
            )
    ...
    def is_cart_empty(self) -> bool:
        try:
            self.wait.until(
                EC.presence_of_element_located((By.XPATH, self.EMPTY_CART))
            )
            return True
        except TimeoutException:
    ...
    def get_cart_count(self) -> int:
        try:
            element = self.wait.until(
                EC.presence_of_element_located((By.XPATH, self.CART_COUNT))
            )
